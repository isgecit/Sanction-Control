...
var script_scApproval = {
    ...
    validate_RequestID: function(sender) {
      ...
      this.validated_FK_SC_Approval_RequestID_main = true;
      this.validate_FK_SC_Approval_RequestID(sender,Prefix);
      },
    ...
    validate_FK_SC_Approval_RequestID: function(o,Prefix) {
      var value = o.id;
      var RequestID = $get(Prefix + 'RequestID');
      if(RequestID.value==''){
        if(this.validated_FK_SC_Approval_RequestID_main){
      ...
      value = value + ',' + RequestID.value ;
        ...
        o.style.backgroundRepeat= 'no-repeat';
        o.style.backgroundPosition = 'right';
        PageMethods.validate_FK_SC_Approval_RequestID(value, this.validated_FK_SC_Approval_RequestID);
      },
    validated_FK_SC_Approval_RequestID_main: false,
    validated_FK_SC_Approval_RequestID: function(result) {
      var p = result.split('|');
      var o = $get(p[1]);
      if(script_scApproval.validated_FK_SC_Approval_RequestID_main){
        var o_d = $get(p[1]+'_Display');
        try{o_d.innerHTML = p[2];}catch(ex){}
      }
      o.style.backgroundImage  = 'none';
