...
Partial Class GF_scProject
  Inherits SIS.SYS.GridBase
  Private _InfoUrl As String = "~/SC_Main/App_Display/DF_scProject.aspx"
  ...
  End Sub
  Protected Sub GVscProject_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles GVscProject.RowCommand
    If e.CommandName.ToLower = "lgedit".ToLower Then
      Try
        Dim RequestID As Int32 = GVscProject.DataKeys(e.CommandArgument).Values("RequestID")  
        Dim SerialNo As Int32 = GVscProject.DataKeys(e.CommandArgument).Values("SerialNo")  
        Dim RedirectUrl As String = TBLscProject.EditUrl & "?RequestID=" & RequestID & "&SerialNo=" & SerialNo
        Response.Redirect(RedirectUrl)
    ...
    End If
  End Sub
  Protected Sub GVscProject_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles GVscProject.Init
    DataClassName = "GscProject"
    SetGridView = GVscProject
  End Sub
  ...
  Protected Sub Page_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
    ...
    Dim validateScriptRequestID As String = "<script type=""text/javascript"">" & _
      ...
      "    validated_FK_SC_Project_RequestID_main = true;" & _
      "    validate_FK_SC_Project_RequestID(o);" & _
      "  }" & _
      "</script>"
    ...
    End If
    Dim validateScriptFK_SC_Project_RequestID As String = "<script type=""text/javascript"">" & _
      "  function validate_FK_SC_Project_RequestID(o) {" & _
      "    var value = o.id;" & _
      "    var RequestID = $get('" & F_RequestID.ClientID & "');" & _
      "    try{" & _
      "    if(RequestID.value==''){" & _
      "      if(validated_FK_SC_Project_RequestID.main){" & _
      ...
      "    PageMethods.validate_FK_SC_Project_RequestID(value, validated_FK_SC_Project_RequestID);" & _
      "  }" & _
      "  validated_FK_SC_Project_RequestID_main = false;" & _
      "  function validated_FK_SC_Project_RequestID(result) {" & _
      ...
      "  }" & _
      "</script>"
    If Not Page.ClientScript.IsClientScriptBlockRegistered("validateFK_SC_Project_RequestID") Then
      Page.ClientScript.RegisterClientScriptBlock(GetType(System.String), "validateFK_SC_Project_RequestID", validateScriptFK_SC_Project_RequestID)
    End If
  End Sub
  <System.Web.Services.WebMethod()> _
