...
Partial Class GF_scRApproval
  ...
  End Sub
  Protected Sub GVscRApproval_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles GVscRApproval.RowCommand
    If e.CommandName.ToLower = "lgedit".ToLower Then
      Try
        Dim RequestID As Int32 = GVscRApproval.DataKeys(e.CommandArgument).Values("RequestID")  
        Dim SerialNo As Int32 = GVscRApproval.DataKeys(e.CommandArgument).Values("SerialNo")  
        Dim RedirectUrl As String = TBLscRApproval.EditUrl & "?RequestID=" & RequestID & "&SerialNo=" & SerialNo
        Response.Redirect(RedirectUrl)
    ...
    If e.CommandName.ToLower = "approvewf".ToLower Then
      ...
      Try
        Dim ApproverRemarks As String = CType(GVscRApproval.Rows(e.CommandArgument).FindControl("F_ApproverRemarks"), TextBox).Text
        Dim RequestID As Int32 = GVscRApproval.DataKeys(e.CommandArgument).Values("RequestID")
        Dim SerialNo As Int32 = GVscRApproval.DataKeys(e.CommandArgument).Values("SerialNo")
        SIS.SC.scRApproval.ApproveWF(RequestID, SerialNo, ApproverRemarks)
        GVscRApproval.DataBind()
      Catch ex As Exception
        ScriptManager.RegisterClientScriptBlock(Page, Page.GetType(), "", "alert('" & New JavaScriptSerializer().Serialize(ex.Message) & "');", True)
      End Try
    End If
    If e.CommandName.ToLower = "rejectwf".ToLower Then
      Try
        ...
        Dim RequestID As Int32 = GVscRApproval.DataKeys(e.CommandArgument).Values("RequestID")  
        Dim SerialNo As Int32 = GVscRApproval.DataKeys(e.CommandArgument).Values("SerialNo")  
        SIS.SC.scRApproval.RejectWF(RequestID, SerialNo, ApproverRemarks)
        GVscRApproval.DataBind()
      Catch ex As Exception
        ScriptManager.RegisterClientScriptBlock(Page, Page.GetType(), "", "alert('" & New JavaScriptSerializer().Serialize(ex.Message) & "');", True)
      End Try
    ...
    If e.CommandName.ToLower = "completewf".ToLower Then
      ...
      Try
        ...
        Dim RequestID As Int32 = GVscRApproval.DataKeys(e.CommandArgument).Values("RequestID")
        Dim SerialNo As Int32 = GVscRApproval.DataKeys(e.CommandArgument).Values("SerialNo")
        SIS.SC.scRApproval.CompleteWF(RequestID, SerialNo, ApproverRemarks)
        GVscRApproval.DataBind()
      Catch ex As Exception
        ScriptManager.RegisterClientScriptBlock(Page, Page.GetType(), "", "alert('" & New JavaScriptSerializer().Serialize(ex.Message) & "');", True)
      End Try
    End If
  End Sub
  Protected Sub GVscRApproval_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles GVscRApproval.Init
    DataClassName = "GscRApproval"
    SetGridView = GVscRApproval
  End Sub
